...
class Args:
    ...
    # Safety. By default this script only runs inference and prints actions.
    execute: bool = False
    yes: bool = False
    max_joint_velocity: float = 1.0
    policy_velocity_scale: float = 1.0
    franky_dynamics_factor: float = 1.0
    franky_velocity_motion_duration: float = 0.25
...
class FrankaController:
    def __init__(self, args: Args):
        ...
        self._max_joint_velocity = args.max_joint_velocity
        self._policy_velocity_scale = args.policy_velocity_scale
        self._velocity_motion_duration = args.franky_velocity_motion_duration
    ...
    def step(self, action: np.ndarray, *, duration_s: float, asynchronous: bool) -> np.ndarray:
        action = np.asarray(action, dtype=np.float64)
        joint_velocity = self._clip_joint_velocity(action)
        motion_duration_s = max(duration_s, self._velocity_motion_duration if asynchronous else duration_s)
        duration_ms = max(1, round(motion_duration_s * 1000))
        motion = self._franky.JointVelocityMotion(joint_velocity.tolist(), duration=self._franky.Duration(duration_ms))
    ...
    def execute_action_sequence(self, actions: np.ndarray, *, period_s: float) -> np.ndarray:
        ...
        joint_velocities = np.stack([self._clip_joint_velocity(action) for action in actions])
        period_ms = max(1, round(period_s * 1000))
        waypoints = [
            self._franky.JointVelocityWaypoint(
                velocity,
                hold_target_duration=self._franky.Duration(period_ms),
            )
            for velocity in joint_velocities
        ]
        motion = self._franky.JointVelocityWaypointMotion(waypoints)
    ...
    def stop(self) -> None:
        with contextlib.suppress(Exception):
            self._robot.move(self._franky.JointVelocityStopMotion())
    ...
    def _clip_joint_velocity(self, action: np.ndarray) -> np.ndarray:
        return np.clip(
            action[:7] * self._policy_velocity_scale,
...
def main(args: Args) -> None:
    ...
    if args.execute and not args.yes:
        confirm = input("Type EXECUTE to send policy actions to the Franka: ")
        if confirm != "EXECUTE":
            raise SystemExit("Aborted before enabling robot execution.")
            ...
            for t_step in range(args.max_timesteps):
                ...
                if action_chunk is None or actions_from_chunk_completed >= args.open_loop_horizon:
                    ...
                    print(
                        ...
                        f"first_joint_vel={np.round(action_chunk[0, :7], 3)} gripper={action_chunk[0, 7]:.3f}"
                    )
                ...
                if args.execute:
                    sent_velocity = controller.step(action, duration_s=period_s, asynchronous=not args.step_on_space)
                    if args.step_on_space:
                        print(f"executed clipped_joint_vel={np.round(sent_velocity, 3)} gripper={action[7]:.3f}")
                else:
                    sent_velocity = np.clip(
                        action[:7] * args.policy_velocity_scale,
                ...
                if not args.execute and (args.step_on_space or t_step % args.open_loop_horizon == 0):
                    print(f"dry_run clipped_joint_vel={np.round(sent_velocity, 3)} gripper={action[7]:.3f}")
    ...
    *,
    ...
    while t_step < args.max_timesteps:
        ...
        print(
            ...
            f"first_joint_vel={np.round(actions[0, :7], 3)} gripper={actions[0, 7]:.3f}"
        )
        sent_velocities = controller.execute_action_sequence(actions, period_s=period_s)
        print(
            f"executed first_clipped_joint_vel={np.round(sent_velocities[0], 3)} "
            f"last_clipped_joint_vel={np.round(sent_velocities[-1], 3)}"
        )
        t_step += horizon
